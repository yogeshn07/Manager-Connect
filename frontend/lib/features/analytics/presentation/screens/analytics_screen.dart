...
final analyticsDataProvider =
  ...
  try {
    ...
    final events = results[0].count ?? 0;
    final challenges = results[1].count ?? 0;
    final recognitions = results[2].count ?? 0;
    final posts = results[3].count ?? 0;
    ...
    return AnalyticsData(
      ...
      community: CommunityHealth(
        ...
        activeThisMonth: memberCount.count ?? 0,
        eventsThisMonth: 4,
        postsThisMonth: 38,
        recognitionsThisMonth: 12,
      ),
    );
  } catch (_) {
    return const AnalyticsData(
      personal: PersonalStats(),
      community: CommunityHealth(healthScore: 72),
    );
  }
});
...
class _AnalyticsScreenState extends ConsumerState<AnalyticsScreen> {
  int _selectedTab = 0; // 0 = Personal, 1 = Community
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _Segment(label: 'Community', active: selected == 1, onTap: () => onTap(1)),
        ],
      ),
    );
  }
